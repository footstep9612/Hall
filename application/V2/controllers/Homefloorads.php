 ...
 * @desc
 */
class HomeflooradsController extends PublicController {
    ...
    public function ListAction() {
        ...
        if ($list) {
            $count = $home_floor_ads_model->getCont($condition);
    ...
    public function InfoAction() {
        ...
        $list = $home_floor_ads_model->getInfo($id);
        if ($list) {
            $this->jsonReturn($list);
        } elseif ($list === null) {
            $this->setCode(MSG::ERROR_EMPTY);
    ...
    public function CreateAction() {
        ...
        if ($home_floor_ads_model->getExit($condition)) {
            $this->setCode(MSG::MSG_EXIST);
            $this->setMessage('所在国家已经存在相同楼层名称,请您添加不同名称的楼层广告!');
            $this->jsonReturn();
        }


        $list = $home_floor_ads_model->createData($condition);
        if ($list) {
            $this->jsonReturn($list);
        } elseif ($list === false) {
            $this->setCode(MSG::ERROR_EMPTY);
    ...
    public function UpdateAction() {
        ...
        if ($home_floor_ads_model->getExit($condition, $id)) {
            $this->setCode(MSG::MSG_EXIST);
        ...
        $list = $home_floor_ads_model->updateData($id, $condition);
        if ($list) {
            $this->jsonReturn();
        } elseif ($list === false) {
            $this->setCode(MSG::ERROR_EMPTY);
    ...
    public function DeletedAction() {
        ...
        $list = $home_floor_ads_model->DeletedData($id);
        if ($list) {
            $this->jsonReturn($list);
        } elseif ($list === false) {
            $this->setCode(MSG::ERROR_EMPTY);
