...
class BuyerbusinessController extends PublicController
{
    ...
    public function createBusinessAction()
    {
        ...
        if(!empty($data['purchase'])){
            ...
            $purchaseRes = $purchase->createPurchase($data['purchase'],$data['created_by']);
        }
        if(!$businessRes || !$purchaseRes){
            echo json_encode(array("code" => "0", "data" => $res, "message" => "创建失败"));
            exit();
        }
        echo json_encode(array("code" => "1", "data" => $res, "message" => "创建成功"));
    }
}
