...
class OrderlogController extends PublicController {
    ...
    public function addAction() {
        ...
        if (in_array($data['log_group'], ['CREDIT', 'COLLECTION'])) {
            if ($data['amount'] < 0) {
                $this->setCode(MSG::ERROR_PARAM);
                $this->setMessage('金额不能为负数');
                $this->jsonReturn(null);
            } elseif (!is_numeric($data['amount'])) {
                $this->setCode(MSG::ERROR_PARAM);
                $this->setMessage('金额不能为非数字');
                $this->jsonReturn(null);
            }
        }
        $OrderLog->startTrans();
        ...
        if ($results['code'] == 1) {
            if ($data['log_group'] === 'CREDIT') {
                $buyer_model = new BuyerModel();
                $rs = $buyer_model->updateCredite($data['order_id'], $data['type'], $data['amount']);
                if ($rs['code'] !== 1) {
                    $OrderLog->rollback();
                    $this->jsonReturn($rs);
                }
