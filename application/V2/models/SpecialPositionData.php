    ...
    public function getList($condition){
        ...
        try{
            ...
            $rel = $this->field("$thistable.id,$thistable.lang,$thistable.special_id,$thistable.position_id,$thistable.sku,$thistable.sort_order,$thistable.created_at,$thistable.created_by,$thistable.spu,$ptable.name,$ptable.show_name,attach.attach_url")
                ...
                ->join("(SELECT spu,MAX(sort_order),attach_url,default_flag,deleted_flag,status FROM $patable WHERE default_flag='Y' AND deleted_flag='N' AND status='VALID' GROUP BY spu) as attach ON $thistable.spu=attach.spu","LEFT")
                ->where($where)
                ->limit($from, $size)
                ->select();
            if($rel){
