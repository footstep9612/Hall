        ...
        if ($exist) {
            ...
            if (empty($attr['spec_attrs']) && !$other_attr['spec_attrs']) {
                if ($boolen) {
                    ...
                    jsonReturn('', ErrorMsg::EXIST, $condition['sku'][1] .' 名称：[' . $condition['name'] . '],型号：[' . $condition['model'] . ']已存在');
                }
            } else {
                foreach ($other_attr as $key => $item) {
                    ...
                    if (empty($result1) && empty($result2)) {
                        if ($boolen) {
                            ...
                            jsonReturn('', ErrorMsg::EXIST, $condition['sku'][1] .' 名称：[' . $condition['name'] . '], 型号：[' . $condition['model'] . ']已存在' . '; 扩展属性重复!');
                        }
                    } else {
                        continue;
                    }
