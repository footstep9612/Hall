...
//put your code here
    ...
    private function getCondition($condition, $lang = 'en', &$country_bn = null, &$is_show_cat = false, &$show_cat_name = null, &$is_brand = false, &$brand_name = null) {
        ...
        ESClient::getQureyByArr($condition, $body, ESClient::TERMS, 'spus', 'spu');
        ...
        ESClient::getStatus($condition, $body, ESClient::TERM, 'status', 'status', ['NORMAL', 'VALID', 'TEST', 'CHECKING', 'CLOSED', 'DELETED']);
        ...
        ESClient::getQurey($condition, $body, ESClient::MATCH, 'exe_standard', 'exe_standard.' . $analyzer);
        ESClient::getQurey($condition, $body, ESClient::MATCH, 'app_scope', 'app_scope.' . $analyzer);
        ESClient::getQurey($condition, $body, ESClient::MATCH, 'advantages', 'advantages.' . $analyzer);
        ESClient::getQurey($condition, $body, ESClient::WILDCARD, 'tech_paras', 'tech_paras.all');
        ESClient::getQurey($condition, $body, ESClient::WILDCARD, 'warranty', 'warranty.all');
        ESClient::getQurey($condition, $body, ESClient::MATCH, 'source_detail', 'source_detail.' . $analyzer);
        ESClient::getQurey($condition, $body, ESClient::MATCH, 'keywords', 'keywords.' . $analyzer);
        ESClient::getQurey($condition, $body, ESClient::WILDCARD, 'supplier_id', 'suppliers.all');
        ESClient::getQurey($condition, $body, ESClient::WILDCARD, 'supplier_name', 'suppliers.all');
                ...
                if (empty($brands)) {
                    $body['query']['bool']['must'][] = ['bool' => [ESClient::SHOULD => [
                                ['bool' => [ESClient::MUST => [
                                            ...
                                            [ESClient::WILDCARD => ['show_name.lower' => ['value' => '*' . strtolower($keyword), 'boost' => 1000]]]]]],
                                ['bool' => [ESClient::MUST => [
                                            ['bool' => [ESClient::MUST_NOT => [[ESClient::WILDCARD => ['show_name.lower' => ['value' => '* for *', 'boost' => 150]]]]]],
                                            [ESClient::WILDCARD => ['show_name.lower' => ['value' => strtolower($keyword) . '*', 'boost' => 900]]]]]],
                                ['bool' => [ESClient::MUST => [
                                            ['bool' => [ESClient::MUST_NOT => [[ESClient::WILDCARD => ['show_name.lower' => ['value' => '* for *', 'boost' => 150]]]]]],
                                            [ESClient::WILDCARD => ['show_name.lower' => ['value' => '*' . strtolower($keyword) . '*', 'boost' => 800]]]]]],
                                [ESClient::WILDCARD => ['show_name.lower' => ['value' => '*' . strtolower($keyword), 'boost' => 700]]],
                                [ESClient::WILDCARD => ['show_name.lower' => ['value' => strtolower($keyword) . '*', 'boost' => 600]]],
                                [ESClient::WILDCARD => ['show_name.lower' => ['value' => '*' . strtolower($keyword) . '*', 'boost' => 500]]],
                                ['bool' => [ESClient::MUST => [
                                            ['bool' => [ESClient::SHOULD => [
                                                        [ESClient::TERM => ['recommend_flag' => ['value' => 'Y', 'boost' => 100]]],
                                                        [ESClient::TERM => ['recommend_flag' => ['value' => 'N', 'boost' => 0]]],
                                                    ]]],
                                            [ESClient::MATCH => ['show_name.' . $analyzer => ['query' => $keyword, 'boost' => 100, 'minimum_should_match' => '50%', 'operator' => 'or']]]]]],
                                //[ESClient::MATCH => ['name.' . $analyzer => ['query' => $keyword, 'boost' => 100, 'minimum_should_match' => '50%', 'operator' => 'or']]],
