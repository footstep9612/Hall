...
class ExcelmanagerController extends PublicController {
    ...
    public function downQuotationAction() {
		...
		$condition = ['inquiry_id'=>intval($request['id']),'attach_group'=>'FINAL'];
        ...
        $data = $this->getFinalQuoteData($request['id']);
        //p($data);
        //创建excel表格并填充数据
    ...
    public function downFinalQuotationAction() {
    
        $request = $this->validateRequests('id');

        //更改报价的状态
        $quoteModel = new QuoteModel();
        $quoteModel->where(['inquiry_id'=>$request['id']])->save(['status'=>'COMPLETED']);
        //更改询单的状态
        ...
        $data = $this->getCommercialQuoteData($request['id']);
	...
	**/
    ...
    private function createFinalExcelAndInsertData($quote) {
        ...
        if(!empty($quote['list'])){
            ...
            $_14 = $startRow+2; $_15 = $startRow+3; $_16 = $startRow+4; $_17 = $startRow+5; $_18 = $startRow+6; $_19 = $startRow+7;
