...
class ProductModel extends PublicModel {
  ...
  public function editInfo($input) {
    ...
    try {
      foreach ($input as $key => $item) {
        if (in_array($key, array('zh', 'en', 'ru', 'es'))) {
          ...
          if (!isset($input['spu'])) {
            ...
            $flag = $this->add($data);
            if ($flag && $spu && $data['lang']) {
              $esproduct_model->create_data($data, $data['lang']);
            }
          } else {
            ...
            $data['updated_at'] = date('Y-m-d H:i:s', time());    //修改时间
            $flag = $this->where(array('spu' => trim($input['spu']), 'lang' => $key))->save();
            if ($flag && $spu && $key) {
              $esproduct_model->update_data($data, $input['spu'], $key);
            }
          }
        } elseif ($key == 'attachs') {
          if ($item) {
      ...
      $this->commit();
      if ($spu) {
        $esproduct_model->Update_Attachs($spu, 'en');
        $esproduct_model->Update_Attachs($spu, 'zh');
        $esproduct_model->Update_Attachs($spu, 'es');
        $esproduct_model->Update_Attachs($spu, 'ru');
      }
      return $spu;
    } catch (Exception $e) {
      $this->rollback();
  ...
  public function createSpu($data) {
    ...
    if ($res) {
      if ($condition['spu'] && $condition['lang']) {
        $esproduct_model = new EsproductModel();
        $esproduct_model->create_data($condition, $condition['lang']);
      }
      return true;
    } else {
      return false;
  ...
  public function updateSpu($data, $where) {
    $condition = $this->check_data($data);
    if (!empty($where)) {
      if ($where['spu'] && $where['lang']) {
        $esproduct_model = new EsproductModel();
        $condition['spu'] = $where['spu'] ? $where['spu'] : $condition['spu'];
        $condition['lang'] = $where['lang'] ? $where['lang'] : $condition['lang'];
        $esproduct_model->update_data($condition, $condition['spu'], $condition['lang']);
      }
  ...
  public function deleteSpu($where) {
    if (!empty($where)) {
      if ($condition['spu'] && $condition['lang']) {
        $esproduct_model = new EsproductModel();
        $esproduct_model->delete_data($condition, $condition['lang']);
      }
