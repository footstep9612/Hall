...
class ShowCatModel extends PublicModel {
    ...
    public function updateothercat($old_cat_no, $new_cat_no) {
        $flag = $this->table($this->dbName . '.t_show_cat_product')
        ...
        $es_product_model = new EsproductModel();
        $cats_new = $this->getinfo($data['cat_no'], 'en');
        $cat_old = $this->getinfo($data['cat_no'], 'en');
        $es_product_model->Replaceshowcats($data['cat_no'], json_encode($cat_old, 256), json_encode($cats_new, 256), 'en');
        $cats_new = $this->getinfo($data['cat_no'], 'zh');
        $cat_old = $this->getinfo($data['cat_no'], 'zh');
        $es_product_model->Replaceshowcats($data['cat_no'], json_encode($cat_old, 256), json_encode($cats_new, 256), 'zh');
        $cats_new = $this->getinfo($data['cat_no'], 'es');
        $cat_old = $this->getinfo($data['cat_no'], 'es');
        $es_product_model->Replaceshowcats($data['cat_no'], json_encode($cat_old, 256), json_encode($cats_new, 256), 'es');
        $cats_new = $this->getinfo($data['cat_no'], 'ru');
        $cat_old = $this->getinfo($data['cat_no'], 'ru');
        $es_product_model->Replaceshowcats($data['cat_no'], json_encode($cat_old, 256), json_encode($cats_new, 256), 'ru');
        return true;
    }
