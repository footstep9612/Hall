...
trait QuoteHelper {
    ...
    public static function quoteListHandler($inquiry_id, $rol = 'QUOTER') {
        ...
        foreach ($quoteItemList as $item => $value) {
            $quoteItemList[$item]['supplier_name'] = Z('erui_supplier.Supplier')->where(['id' => $value['supplier_id']])->getField('name');
    ...
    public static function bizlineManagerQuoteList($condition) {
        ...
        return $quoteBizlineModel->alias('qb')
                        ->join('erui_rfq.inquiry inq ON qb.inquiry_id = inq.id')
                        ->field($field)
                        ->where($where)
    ...
    public static function bizlineManagerQuoteSkuList($condition) {
        ...
        $data = $quoteItemModel->alias('a')
                ...
                ->join('erui_rfq.quote_bizline qb ON a.quote_id = qb.quote_id and a.bizline_id=qb.bizline_id')
                ->field($fields)
                ->where($where)
                ->order('a.id DESC')
                ->select();
...
//            ->count('a.id');
        ...
        $count = $quoteItemModel->alias('a')
                ->join('erui_rfq.inquiry_item b ON a.inquiry_item_id = b.id')
                ->join('erui_rfq.quote_bizline qb ON a.quote_id = qb.quote_id')
                ->field($fields)
                ->where($where)
                ->count('a.id');
        return $count > 0 ? $count : 0;
    ...
    public static function getQuoteList($condition) {
        ...
        return $quoteItem->alias('a')
                        ->join('erui_rfq.inquiry inq ON a.inquiry_id = inq.id', 'LEFT')
                        ->field($fields)
                        ->where($where)
                        ->order('a.id DESC')
                        ->select();
    ...
    public static function getManagerQuoteSkuList($condition) {
        ...
        return $quoteItem->alias('a')
                        ...
                        ->join('erui_rfq.inquiry_item c ON a.inquiry_item_id = c.id', 'LEFT')
